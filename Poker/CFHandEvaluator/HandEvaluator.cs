...
namespace Poker.CFHandEvaluator
{
    ...
    public partial class Hand : IComparable
    {
		...
		public static string HandCards(ulong cards)
		{
			string[] cardTbl = { "2", "3", "4", "5", "6", "7", "8", "9", "T", "J", "Q", "K", "A" };
			...
			{
				var cardmask = Hand.Mask(rank);
				if ((ss & cardmask) == cardmask)
				{
					result += cardTbl[rank] + "s, ";
				}
				if ((sh & cardmask) == cardmask)
				{
					result += cardTbl[rank] + "h, ";
				}
				if ((sd & cardmask) == cardmask)
				{
					result += cardTbl[rank] + "d, ";
				}
				if ((sc & cardmask) == cardmask)
				{
					result += cardTbl[rank] + "c, ";
				}
			}
		...
		{
			string result = "";
			foreach (var card in cards) { result += HandCards(card); }
			return result;
		}
...
#endif
			string[] cardTbl = { "2", "3", "4", "5", "6", "7", "8", "9", "T", "J", "Q", "K", "A" };
			string[] suitTbl = { "c", "d", "h", "s" };

			return cardTbl[card % 13] + suitTbl[card / 13];
		}
